...
export const DemoOpener: React.FC<DemoOpenerProps> = ({channelName, avatarUrl}) => {
  ...
  return (
    <AbsoluteFill style={{overflow: 'hidden', background: '#35204e', fontFamily: 'Arial, Helvetica, sans-serif'}}>
      ...
      <div style={{position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 30, padding: '0 80px'}}>
        <div style={{width: 118, height: 118, borderRadius: '50%', border: '3px solid rgba(255,255,255,.92)', flexShrink: 0, overflow: 'hidden', scale: interpolate(badgeEntry, [0, 1], [0.4, 1]), rotate: `${interpolate(badgeEntry, [0, 1], [-35, 0])}deg`}}>
          <Img src={avatarUrl} alt={`${channelName} avatar`} style={{width: '100%', height: '100%', objectFit: 'cover'}} />
        </div>
        <KineticTitle text={channelName} fontSize={54} maxWidth={310} startFrame={9} />
      </div>
